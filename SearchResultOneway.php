...
global $sql1,$sql2,$result, $availableNumber;
  ...
  if($stop=="nonstop"){
    ...
    //search by city or code for departure and arrival
    $sql1 = "SELECT FL.number AS FLnumber, company, type, departure, d_time, arrival, a_time, C.name AS classname, capacity, price, COUNT(*)
            ...
            WHERE C.name = '$class' AND
            (
                ((AD.city LIKE '%$from%' OR AD.code LIKE '%$from%') AND (AA.city LIKE '%$to%' OR AA.code LIKE '%$to%'))
            )
            GROUP BY FL.number
            HAVING COUNT(*)>1
            ORDER BY FL.number";
    ...
    if(mysqli_num_rows($result))
      ...
      //search by city or code for departure and arrival with different combinations
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        echo '<td>
            ...
            <button type="submit" class="btn btn-success">Select Seats</button>
            </form>
            </td>';
        }else{
